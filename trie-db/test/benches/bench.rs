...
// RocksDB 依赖
use rocksdb::{
	BlockBasedIndexType, BlockBasedOptions, DataBlockIndexType, Options, SliceTransform, DB,
};
...
// 全局共享的测试数据，仅生成一次
static TEST_DATA: Lazy<Vec<(Vec<u8>, Vec<u8>)>> = Lazy::new(|| generate_test_data());
...
fn trie_read_benchmark(c: &mut Criterion) {
	...
	// ---------- 顺序读取测试：使用排序后的 key ----------
	let mut all_keys: Vec<Vec<u8>> =
		test_data.iter().map(|(k, _): &(Vec<u8>, Vec<u8>)| k.clone()).collect();
